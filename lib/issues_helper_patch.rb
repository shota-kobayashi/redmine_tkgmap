	...
	module InstanceMethods
		def render_custom_fields_rows_with_tkg(issue)
			...
			ordered_values.compact.each do |value|
				...
				if(value.custom_field.field_format == Tkgmap::Identifier && show_value(value) =~ /\A-?[0-9\.]+,-?[0-9\.]+\Z/)
					latLng = show_value(value).split(",")
					script ="//<![CDATA[
						$(function(){
							initMap(#{latLng[0]}, #{latLng[1]}, false, #{Setting.plugin_redmine_tkgmap['default_zoom'].to_i});
						});
					//]]>"
				  s << javascript_include_tag('tkgmap_application',:plugin => 'redmine_tkgmap')
					...
					s << content_tag("script", "",{:src =>TkgmapController.helpers.gmap_api_uri, :type =>'text/javascript', :charset=>'UTF-8'})
